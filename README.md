...
| 24 | `scripts/score-bigint-migration.sql` | Migrate `scores.score` to `bigint`, raise ceiling to 100 billion for high pinball scores |
...
- **Profiles**: All cross-user lookups in the app (feed, comments, chat previews, teams, tournament join requests, searches) query the `public_profiles` view, which hides private accounts from other users and exposes only display fields — never `is_admin`, `role`, `email`, `phone`, `is_private`, or `square_customer_id`. The raw `profiles` table is locked down (`profiles-lockdown.sql`): users read only their own row, admins read all, and every cross-user surface goes through `public_profiles` or `rpc_get_public_profile` (which folds privacy + friendship checks in server-side). Users cannot set `is_admin` or `is_arcade_official` on themselves — locked by trigger + WITH CHECK.
